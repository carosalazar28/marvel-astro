...
- La persona usuaria puede avanzar cada ítem de `Sin ver` a `Viendo` y finalmente `Vista`; un ítem ya visto no retrocede por una acción repetida. La interfaz ofrece dos reinicios explícitos: **Reiniciar progreso local** borra las modificaciones del navegador y restaura los estados editoriales del JSON; **Reiniciar calendario completo** guarda localmente todos los ids como `Sin ver`, incluida la base editorial marcada como `Vista`.
